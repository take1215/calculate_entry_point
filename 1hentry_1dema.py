...
def calculate_entry_points(data):
    ...
    higher_timeframe_data = get_historical_data('SOXL', '2023-01-01', '2023-08-01', interval='1d')  # 上位足（例：週足）のデータを取得

    # タイムゾーンを追加してリサンプリング
    higher_timeframe_data = higher_timeframe_data.tz_localize(pytz.timezone('America/New_York')).resample('1h').ffill()
